...
def move_robot():
    ...
    convert to MAT-frame meters, and publish on /suture_cuts as std_msgs/String JSON.
    """
    ...
    # ---------- px → meters (MAT frame) ----------
    # Prefer UI px/mm if provided; else use pad mapper or fallback (mat 120x175 mm)
    if px_per_mm_ui is not None and px_per_mm_ui > 0:
        px_per_mm = float(px_per_mm_ui)
    else:
        try:
            from .image_to_map_mapper import get_px_per_mm
        except Exception:
            # fallback if the helper isn’t present in the install
            def get_px_per_mm(_m, w, h, fb):
                try:
                    return float(fb)
                except Exception:
                    return 3.0
        ppm_fallback = ((float(W) / 120.0) + (float(H) / 175.0)) * 0.5 if (W > 0 and H > 0) else 3.0
        # IMPORTANT: pass W, H as separate numbers (NOT a tuple)
        px_per_mm = get_px_per_mm(PAD_MAPPER, W, H, ppm_fallback)

    if not px_per_mm or px_per_mm <= 0:
        px_per_mm = ((float(W) / 120.0) + (float(H) / 175.0)) * 0.5 if (W > 0 and H > 0) else 3.0
