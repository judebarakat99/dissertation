...
from suture_planner import generate_suture_points
#, export_sutures_to_json, export_sutures_to_csv
    ...
    try:
        while True:
            ...
            for cnt in contours:
                ...
                # Draw wound endpoints
                cv2.circle(color_image, leftmost, 5, (255, 0, 0), -1)
                cv2.circle(color_image, rightmost, 5, (0, 0, 255), -1)

                
                # --- Add Suture Markers ---
                suture_pairs = generate_suture_points(cnt, num_stitches=5, wound_width=20)

                for entry, exit in suture_pairs:
                    cv2.circle(color_image, entry, 4, (0, 255, 255), -1)
                    cv2.circle(color_image, exit, 4, (0, 165, 255), -1)
                    cv2.line(color_image, entry, exit, (0, 128, 255), 1)
                    
                # # --- Add Suture Markers ---
                # suture_points = generate_suture_points(cnt, depth_frame, depth_intrin, num_stitches=5)

                # for s in suture_points:
                #     entry, exit = s['entry'], s['exit']
                #     cv2.circle(color_image, entry, 4, (0, 255, 255), -1)
                #     cv2.circle(color_image, exit, 4, (0, 165, 255), -1)
                #     cv2.line(color_image, entry, exit, (0, 128, 255), 1)

                # # Optional: export results
                # export_sutures_to_json(suture_points, "wound_sutures.json")
                # export_sutures_to_csv(suture_points, "wound_sutures.csv")
